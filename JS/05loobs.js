...
const initialNumber = parseInt(prompt("Please enter a number. "));
const finalNumber = parseInt(prompt("Please enter a number. "));
...
for (var i = initialNumber; i <= finalNumber; i++) {
  ...
  console.log(`The result of number : ${result}`);
...
for(let i = 1; i <= studentCount; i++){
    const grade = parseInt(prompt(`Enter the grade for student ${i}`));
    sum += grade;
...
console.log(`${studentPass} students have passed the exam`);
